...
def flow_estimation(model,
    ...
    # Check where either translation in x or y is not zero
    non_zero_mask = (flow[..., 0] != 0) | (flow[..., 1] != 0)
    # Get the indexes of non-zero flow
    idxs = np.argwhere(non_zero_mask)

    mkpts0, mkpts1 = flow_to_correspondences(flow, idxs)
...
def inference_flow(model,
    ...
    for test_id in range(0, len(filenames) - 1):
        ...
        # mkpts0, mkpts1 = flow_to_correspondences(flow)
        ...
        # ax.imshow(img_concat)
        # # Adjust the keypoints in img2 to account for the concatenated layout
        # keypoints_img2_adjusted = [(x + img1.shape[1], y) for (x, y) in mkpts1]
        # # Plot keypoints and lines
        # for (pt1, pt2) in zip(mkpts0, keypoints_img2_adjusted):
        #     # Plot circles at the keypoints
        #     ax.plot(pt1[0], pt1[1], 'wo', markersize=3)  # Green circles in image1
        #     ax.plot(pt2[0], pt2[1], 'wo', markersize=3)  # Green circles in image2
        #     # Plot lines between keypoints
